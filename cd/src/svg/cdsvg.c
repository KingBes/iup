...
static void cdftext(cdCtxCanvas *ctxcanvas, double x, double y, const char *str, int len)
{
  char* anchor;
  char* alignment;
  ...
  if (ctxcanvas->canvas->text_orientation)
  {
    ...
    fprintf(ctxcanvas->file, "%s", ctxcanvas->utf8_buffer);
...
static int cdhatch(cdCtxCanvas *ctxcanvas, int style)
{
  ...
  if (ctxcanvas->canvas->back_opacity==CD_OPAQUE)
  {
    fprintf(ctxcanvas->file, "<rect x=\"0\" y=\"0\" width=\"%d\" height=\"%d\" style=\"fill:%s; stroke:none; opacity:%g\" />\n",
            hsize, hsize, ctxcanvas->bgColor, ctxcanvas->bg_opacity);
  }
  ...
  for (j = 0; j < m; j++)
  {
    for (i = 0; i < n; i++)
    {
      ...
      if (ctxcanvas->canvas->invert_yaxis)
        ret = data2rgb(ctxcanvas, n, i, m-1 - j, data, &r, &g, &b, &a);
      else
        ret = data2rgb(ctxcanvas, n, i, j, data, &r, &g, &b, &a);
...
static int long2rgb(cdCtxCanvas *ctxcanvas, int n, int i, int j, void* data, unsigned char*r, unsigned char*g, unsigned char*b, unsigned char*a)
{
  ...
  (void)ctxcanvas;
  cdDecodeColorAlpha(long_data[j*n+i], r, g, b, a);
  return 1;
}
...
static int uchar2rgb(cdCtxCanvas *ctxcanvas, int n, int i, int j, void* data, unsigned char*r, unsigned char*g, unsigned char*b, unsigned char*a)
{
  ...
  if (uchar_data[j*n+i])
    cdDecodeColorAlpha(ctxcanvas->canvas->foreground, r, g, b, a);
  else
  {
    if (ctxcanvas->canvas->back_opacity==CD_TRANSPARENT)
      return -1;
    else
      cdDecodeColorAlpha(ctxcanvas->canvas->background, r, g, b, a);
  }
...
{
  unsigned char r, g, b, a;
  cdDecodeColorAlpha(color, &r, &g, &b, &a);

  if (a != 255)
    ctxcanvas->bg_opacity = (double)a / 255.0;
  else
    ctxcanvas->bg_opacity = ctxcanvas->opacity;
...
static long cdforeground(cdCtxCanvas *ctxcanvas, long int color)
{
  ...
  if (a != 255)
    ctxcanvas->opacity = (double)a / 255.0;
...
{
  unsigned char r, g, b, a;
  double opacity = ctxcanvas->opacity;
  cdDecodeColorAlpha(color, &r, &g, &b, &a);
...
static void set_opacity_attrib(cdCtxCanvas *ctxcanvas, char* data)
{
  if (data)
  {
    ...
    if (opacity < 0) 
      ctxcanvas->opacity = 0.0;
    else if (opacity > 255) 
      ctxcanvas->opacity = 1.0;
    else 
      ctxcanvas->opacity = (double)opacity/255.0;
  }
  else
    ctxcanvas->opacity = 1.0;
...
static void cdcreatecanvas(cdCanvas *canvas, void *data)
{
  ...
  w_pt = CD_MM2PT*canvas->w_mm;
  h_pt = CD_MM2PT*canvas->h_mm;
  ...
  /* SVG specification states that number must use dot as decimal separator */
